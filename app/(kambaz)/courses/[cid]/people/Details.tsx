...
export default function PeopleDetails({ uid, onClose }: { uid: string | null; onClose: () => void; }) {
  ...
  const deleteUser = async (uid: string) => {
    ...
    const ROLES = ["STUDENT", "FACULTY", "TA", "ADMIN"];
  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <hr />
      <b>Email:</b>     
       {!editing ? ( <><span className="wd-email"> {user.email}</span><br/></>
       ):(
       <FormControl
          type="email"
          className="mt-1 mb-2 wd-edit-email"
          value={email}
      ...
      <b>Role:</b>  
         {!editing ? (
            <><span className="wd-role">       {user.role}      </span> <br /></>
         ) : (
         <FormSelect
          className="mt-1 mb-2 wd-edit-role"
          value={role}
