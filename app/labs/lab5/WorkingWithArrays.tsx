...
export default function WorkingWithArrays() {
  ...
  return (
    ...
    <FormControl defaultValue={todo.id} className="w-50" onChange={
     ...
     <h3>Updating a Todo Completion Status</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Todo Completion</a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <input defaultChecked={todo.completed} className="form-check-input float-start"
             onChange={(e) => setTodo({ ...todo, completed: e.target.checked }) } type="checkbox"/>
      <br /><br /><hr />

    <h3>Updating a Todo Description</h3>
      <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-primary float-end">
        Update Todo Description</a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <FormControl defaultValue={todo.description} className="form-text-input float-start"
             onChange={(e) => setTodo({ ...todo, description: e.target.value }) } type="text"/>
      <br /><br /><hr />
